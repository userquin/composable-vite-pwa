...
export function addPWAIconsPluginTemplate(
  ...
  if (nuxt4) {
    addTemplate({
      filename: 'pwa-icons/plugin.d.ts',
      write: true,
...
`,
    ...
    addPWAIconPluginTemplate(pwaAssetsEnabled, 'all')
  }
  else {
    addPWAIconPluginTemplate(pwaAssetsEnabled, 'client')
    addPWAIconPluginTemplate(pwaAssetsEnabled, 'server')
  }
}
...
function addPWAIconPluginTemplate(
  ...
  if (mode !== 'all') {
    addTemplate({
      filename: `pwa-icons/plugin-${mode}.d.ts`,
      write: true,
      getContents: () => `// Generated by @vite-pwa/nuxt
import type { Plugin } from '#app';
import type { PWAIcons } from '#build/pwa-icons';
...
`,
  ...
  if (pwaAssetsEnabled) {
    addPluginTemplate({
      filename: `pwa-icons/${filename}.js`,
